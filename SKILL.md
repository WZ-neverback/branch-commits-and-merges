...
```
/commit-and-merge --<目标分支名> ["提交信息" 或 '提交信息'] [--no-verify]
```

> **参数顺序灵活**：所有参数**可任意组合**和**调换位置**：
> - `/commit-and-merge --feature/x "message"`
> - `/commit-and-merge "message" --feature/x`
> - 分支参数可省略（默认 `--develop`）
> - commit message 可省略（进入第 5 步询问）
> - `--no-verify` 可省略（默认执行 lint 和 git hooks）
> - 三者可任意组合使用
>
...
| 参数 | 说明 | 默认行为 | 是否必填 |
|------|------|---------|----------|
| `--no-verify` | 绕过 Git 提交钩子（`pre-commit`、`commit-msg`）和 Lint 检查 | 执行 lint + git hooks | ❌ |

> - 传入 `--no-verify` 后：
>   - 第 3 步 Lint 检查**自动跳过**（标记 `SKIP_LINT`）
>   - 第 5.4 步提交命令追加 `--no-verify` 参数，跳过 `pre-commit` 和 `commit-msg` 钩子
> - **适用场景**：钩子卡住、紧急修复、已知 lint 误报等
> - **风险提示**：跳过检查可能导致不规范的代码被提交，建议在汇总报告中明确标注
...
- `TARGET_BRANCH`：扫描 `--<任意文本>` 模式（以 `--` 开头，后跟非空字符直到空格或字符串结束）
  ...
  - 命中 `--no-verify` → 标记为选项标志，**不作为分支名**，继续扫描下一个 `--` 参数
  - 命中其他 `--xxx` → 视为自定义分支名，直接使用
  - 未命中 → 默认 `develop`
- `COMMIT_MESSAGE`：扫描成对出现的 `"..."` 或 `'...'`，命中即取其内部内容；未命中则后续步骤询问
- `NO_VERIFY`：检测是否包含 `--no-verify`，命中则标记 `NO_VERIFY=true` 并自动标记 `SKIP_LINT`

> **解析规则**：
> - 分支参数、commit message 参数、`--no-verify` 标志**位置可互换**
> - 三者可任意组合使用
> - 都不传则使用默认值（develop）+ 第 5 步询问 message
> - 若同时传入多个 `--` 开头参数（如 `--develop --preview`），提示用户参数冲突并停止（`--no-verify` 除外，它可与分支参数共存）
> - `--no-verify` 与分支参数不冲突，但**不能同时出现多个分支参数**
...
> **跳过条件**：`MERGE_ONLY=true`（仅合并）或 `SKIP_LINT` 已标记（node_modules 未安装 或 `NO_VERIFY=true`）
...
- Lint 检查：通过 ✅ / 跳过（未配置）⏭️ / 跳过（node_modules 未安装）⏭️ / 跳过（用户选择）⚠️ / 跳过（--no-verify）⏭️ / 失败 ❌
- 文件变更：N 个文件（仅已暂存）
- Commit 状态：已提交 ✅ / 已提交（--no-verify）✅ / 已取消 ❌
...
- 源分支 = 目标分支时自动跳过合并
- `--no-verify` 跳过 lint 和 git hooks 时，在汇总报告中明确标注，提醒用户本次提交未经检查
