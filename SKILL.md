...
- 告知用户工作区干净，**默认执行仅合并**
- 自动标记 `MERGE_ONLY=true`，**跳过第 2~5 步**（无变更可提交，无需 lint、提交、推送源分支），直接进入第 6 步推送当前分支（若有未推送 commit）或第 7 步切换目标分支
...
- Lint 通过 → 继续执行第 3 步
- Lint 失败（代码问题）：
  1. 自动尝试修复（如 `npm run lint -- --fix` 或项目配置的修复命令）
  2. 修复后重新运行 lint
  3. 若修复成功 → 继续执行第 3 步
  4. 若无法自动修复 → 提示用户 lint 错误，**自动跳过 lint 检查**，附带警告继续执行
...
- **情况 A**：第 0 步已通过 `"..."` 或 `'...'` 提供 → **无需确认，直接使用该 message**
...
3. 在汇总报告中标注：`Stash 恢复：已恢复 ✅ / 恢复冲突 ⚠️ / 未恢复（保留 stash）📦 / 无 stash ✅`
